...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#ffffff', '--archive-text': '#050505', '--archive-surface': '#ffffff', '--archive-accent': '#ef1d2f' } as CSSProperties
  const categoryLabel = category === 'all' ? 'All categories' : CATEGORY_OPTIONS.find((item) => item.slug === category)?.name || category

  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.55fr] lg:px-8 lg:py-16">
          <div className="border-b-4 border-black bg-[var(--archive-surface)] pb-8">
            <div className="inline-flex items-center gap-2 bg-black px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-white"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="mt-5 max-w-4xl text-5xl font-black leading-[0.98] tracking-normal sm:text-6xl">{voice?.headline || `Browse ${label}`}</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 opacity-70">{voice?.description || SITE_CONFIG.description}</p>
            <div className="mt-6 border border-black/10 bg-[var(--slot4-gray)] p-4 text-sm font-bold leading-7 opacity-75">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="bg-[var(--archive-text)] px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-[var(--archive-bg)]">Browse all</Link>
              <Link href="/search" className="border border-black px-5 py-3 text-xs font-black uppercase tracking-[0.14em]">Search posts</Link>
            </div>
          </div>

          <form action={basePath} className="self-end border border-black bg-white p-5">
            ...
            <select name="category" defaultValue={category} className="mt-4 h-12 w-full border border-black bg-white px-4 text-sm font-bold outline-none">
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="mt-3 h-12 w-full bg-[var(--archive-text)] text-sm font-black uppercase tracking-[0.14em] text-[var(--archive-bg)]">Apply</button>
        ...
        <section className="mx-auto max-w-[1180px] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            ...
            <div className="border border-dashed border-black/20 bg-white p-10 text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="border border-black bg-white px-5 py-3 text-sm font-black">Previous</Link> : null}
            ...
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="border border-black bg-white px-5 py-3 text-sm font-black">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden bg-white transition hover:-translate-y-0.5">
