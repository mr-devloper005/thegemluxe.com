...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden bg-white ${dc.motion.lift}`}>
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group block min-w-0 border border-black/10 bg-white p-5 ${dc.motion.lift}`}>
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group grid min-w-0 gap-6 overflow-hidden border-b border-black/10 bg-white pb-10 ${dc.motion.lift} sm:grid-cols-[280px_minmax(0,1fr)]`}>
      <div className={`${dc.media.frame} aspect-[16/10] sm:aspect-auto sm:min-h-[210px]`}>
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
      <div className="min-w-0 py-2 sm:py-4 sm:pr-5">
        <p className={`${dc.type.eyebrow} ${pal.accentText}`}>{getEditableCategory(post)} / Read {String(index + 1).padStart(2, '0')}</p>
        ...
        <span className={`mt-5 inline-flex items-center gap-2 bg-black px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-white`}>Open article <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
